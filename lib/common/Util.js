...
const { HttpCodes, ALPHA_NUMERIC_CHAR } = require('../../config/Constants');
const { logApiError, logMysqlError } = require('../../logger/customLogs');
  ...
  const errorResponse = (err, req, res, next) => {
    const responseObject = formatResponse(HttpCodes.SERVER_ERROR, err.message, true);
    if (res.headersSent) {
      return next(err);
    }
    if (
   ...
   */
  const encryptPasswordSHA1 = (password, salt)  => sha1(salt + (sha1(salt + sha1(password))));
